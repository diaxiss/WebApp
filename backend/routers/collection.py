...
def fetch_user_collection_id(id: str, limit: int = 10, offset: int = 0, token: Optional[str] = Depends(oauth2_scheme)):
    if token and token != "null":
        result = get_user_collection(sub=id, self=False, limit=limit, offset=offset, viewer=decode_token(token).get('sub'))
    else:
        result = get_user_collection(sub=id, self=False, limit=limit, offset=offset, viewer=None)
    cards = result['cards']
    return {'cards': cards}
