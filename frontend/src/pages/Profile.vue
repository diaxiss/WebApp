...
<template>
    ...
    <div>
        ...
        <router-link 
            :to="`/wishlist/${user_id || ''}`"
            v-if="loading===false && wishlist.length!==0">
            <button>View whole wishlist</button>
        </router-link>
        <p 
        v-if="wishlist.length===0 && loading===false">
            {{ user_id ? 'There are' : 'You have'}} no items on {{ user_id ? 'this': 'your'}} wishlist</p>
    </div>
</template>